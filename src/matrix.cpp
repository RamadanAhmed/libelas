...
Matrix::Matrix(const int32_t m_, const int32_t n_) { allocateMemory(m_, n_); }
...
Matrix Matrix::reshape(const Matrix &M, int32_t m_, int32_t n_) {
    if(M.m * M.n != m_ * n_) {
        cerr << "ERROR: Trying to reshape a matrix of size (" << M.m << "x" << M.n << ") to size (" << m_ << "x" << n_ << ")" << endl;
        exit(0);
    }
    Matrix M2(m_, n_);
...
Matrix Matrix::operator/(const Matrix &M) {
    ...
    if(A.m == B.m && A.n == B.n) {
        ...
        cerr << "ERROR: Trying to divide matrices of size (" << A.m << "x" << A.n << ") and (" << B.m << "x" << B.n << ")" << endl;
        exit(0);
    }
}
