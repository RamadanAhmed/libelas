...
#define infect(otri) (otri).tri[6] = (triangle)((unsigned long long)(otri).tri[6] | (unsigned long long)2l)

#define uninfect(otri) (otri).tri[6] = (triangle)((unsigned long long)(otri).tri[6] & ~(unsigned long long)2l)
...
void trifree(int *memptr) { free(memptr); }
...
void poolrestart(struct memorypool *pool) {
    ...
    pool->nextitem = (int *)(alignptr + (unsigned long long)pool->alignbytes - (alignptr % (unsigned long long)pool->alignbytes));
...
void traversalinit(struct memorypool *pool) {
    ...
    pool->pathitem = (int *)(alignptr + (unsigned long long)pool->alignbytes - (alignptr % (unsigned long long)pool->alignbytes));
...
void dummyinit(struct mesh *m, struct behavior *b, int trianglebytes, int subsegbytes) {
    ...
    m->dummytri =
        (triangle *)(alignptr + (unsigned long long)m->triangles.alignbytes - (alignptr % (unsigned long long)m->triangles.alignbytes));
    ...
    if(b->usesegments) {
        ...
        m->dummysub =
            (subseg *)(alignptr + (unsigned long long)m->subsegs.alignbytes - (alignptr % (unsigned long long)m->subsegs.alignbytes));
...
void initializevertexpool(struct mesh *m, struct behavior *b) {
    ...
    m->vertexmarkindex = ((m->mesh_dim + m->nextras) * sizeof(float) + sizeof(int) - 1) / sizeof(int);
    ...
    poolinit(&m->vertices, vertexsize, VERTEXPERBLOCK, m->invertices > VERTEXPERBLOCK ? m->invertices : VERTEXPERBLOCK, sizeof(float));
}
...
void initializetrisubpools(struct mesh *m, struct behavior *b) {
    ...
    poolinit(&m->triangles, trisize, TRIPERBLOCK, (2 * m->invertices - 2) > TRIPERBLOCK ? (2 * m->invertices - 2) : TRIPERBLOCK, 4);
...
vertex getvertex(struct mesh *m, struct behavior *b, int number) {
    ...
    foundvertex = (char *)(alignptr + (unsigned long long)m->vertices.alignbytes - (alignptr % (unsigned long long)m->vertices.alignbytes));
...
float orient3dadapt(vertex pa, vertex pb, vertex pc, vertex pd, float aheight, float bheight, float cheight, float dheight, float permanent) {
    ...
    det += (adheight * ((bdx * cdytail + cdy * bdxtail) - (bdy * cdxtail + cdx * bdytail)) + adheighttail * (bdx * cdy - bdy * cdx)) +
           (bdheight * ((cdx * adytail + ady * cdxtail) - (cdy * adxtail + adx * cdytail)) + bdheighttail * (cdx * ady - cdy * adx)) +
           (cdheight * ((adx * bdytail + bdy * adxtail) - (ady * bdxtail + bdx * adytail)) + cdheighttail * (adx * bdy - ady * bdx));
    if((det >= errbound) || (-det >= errbound)) {
        return det;
    }
...
float orient3d(struct mesh *m, struct behavior *b, vertex pa, vertex pb, vertex pc, vertex pd, float aheight, float bheight, float cheight, float dheight) {
    ...
    permanent = (Absolute(bdxcdy) + Absolute(cdxbdy)) * Absolute(adheight) +
                (Absolute(cdxady) + Absolute(adxcdy)) * Absolute(bdheight) + (Absolute(adxbdy) + Absolute(bdxady)) * Absolute(cdheight);
...
enum locateresult locate(struct mesh *m, struct behavior *b, vertex searchpoint, struct otri *searchtri) {
    ...
    while(totalsamplesleft > 0) {
        ...
        firsttri = (char *)(alignptr + (unsigned long long)m->triangles.alignbytes - (alignptr % (unsigned long long)m->triangles.alignbytes));

        /* Choose `samplesleft' randomly sampled triangles in this block. */
        do {
            sampletri.tri = (triangle *)(firsttri + (randomnation((unsigned int)population) * m->triangles.itembytes));
...
insertvertex(struct mesh *m, struct behavior *b, vertex newvertex, struct otri *searchtri, struct osub *splitseg, int segmentflaws, int triflaws) {
    struct otri horiz;
    struct otri top;
    struct otri botleft, botright;
    struct otri topleft, topright;
                    ...
                    if(enq) {
                        ...
                        if(b->verbose > 2) {
                            printf("  Queueing encroached subsegment (%.12g, %.12g) (%.12g, %.12g).\n",
                                   encroached->subsegorg[0],
                                   encroached->subsegorg[1],
                                   encroached->subsegdest[0],
                                   encroached->subsegdest[1]);
                        }
                    }
                }
...
void triangulatepolygon(struct mesh *m, struct behavior *b, struct otri *firstedge, struct otri *lastedge, int edgecount, int doflip, int triflaws) {
    ...
    if(bestnumber < edgecount - 2) {
        ...
        triangulatepolygon(m, b, &besttri, lastedge, edgecount - bestnumber, 1, triflaws);
...
void vertexmedian(vertex *sortarray, int arraysize, int median, int axis) {
    ...
    if(arraysize == 2) {
        ...
        if((sortarray[0][axis] > sortarray[1][axis]) ||
           ((sortarray[0][axis] == sortarray[1][axis]) && (sortarray[0][1 - axis] > sortarray[1][1 - axis]))) {
            temp = sortarray[1];
            sortarray[1] = sortarray[0];
            sortarray[0] = temp;
        }
    ...
    while(left < right) {
        ...
        do {
            left++;
                ...
                ((sortarray[left][axis] < pivot1) || ((sortarray[left][axis] == pivot1) && (sortarray[left][1 - axis] < pivot2))));
        /* Search for a vertex whose x-coordinate is too small for the right. */
        do {
            right--;
        } while((left <= right) &&
                ((sortarray[right][axis] > pivot1) || ((sortarray[right][axis] == pivot1) && (sortarray[right][1 - axis] > pivot2))));
        if(left < right) {
            /* Swap the left and right vertices. */
            temp = sortarray[left];
            sortarray[left] = sortarray[right];
...
void regionplague(struct mesh *m, struct behavior *b, float attribute, float area) {
    ...
    while(virusloop != (triangle **)NULL) {
        ...
        for(testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
            ...
            if((neighbor.tri != m->dummytri) && !infected(neighbor) && (neighborsubseg.ss == m->dummysub)) {
                if(b->verbose > 2) {
...
void carveholes(struct mesh *m, struct behavior *b, float *holelist, int holes, float *regionlist, int regions) {
    ...
    if(regions > 0) {
        ...
        for(i = 0; i < regions; i++) {
            ...
            if((regionlist[4 * i] >= m->xmin) && (regionlist[4 * i] <= m->xmax) && (regionlist[4 * i + 1] >= m->ymin) &&
...
void writenodes(struct mesh *m, struct behavior *b, float **pointlist, float **pointattriblist, int **pointmarkerlist) {
    ...
    if((m->nextras > 0) && (*pointattriblist == (float *)NULL)) {
        *pointattriblist = (float *)trimalloc((int)(outvertices * m->nextras * sizeof(float)));
    }
...
void writeelements(struct mesh *m, struct behavior *b, int **trianglelist, float **triangleattriblist) {
    ...
    if(*trianglelist == (int *)NULL) {
        *trianglelist = (int *)trimalloc((int)(m->triangles.items * ((b->order + 1) * (b->order + 2) / 2) * sizeof(int)));
    }
    /* Allocate memory for output triangle attributes if necessary. */
    if((m->eextras > 0) && (*triangleattriblist == (float *)NULL)) {
        *triangleattriblist = (float *)trimalloc((int)(m->triangles.items * m->eextras * sizeof(float)));
    }
    tlist = *trianglelist;
...
void writevoronoi(struct mesh *m,
    ...
    if(*vpointlist == (float *)NULL) {
        *vpointlist = (float *)trimalloc((int)(m->triangles.items * 2 * sizeof(float)));
    }
    /* Allocate memory for Voronoi vertex attributes if necessary. */
    if(*vpointattriblist == (float *)NULL) {
        *vpointattriblist = (float *)trimalloc((int)(m->triangles.items * m->nextras * sizeof(float)));
    }
...
void statistics(struct mesh *m, struct behavior *b) {
    ...
    if(b->verbose) {
        ...
        printf("  Approximate heap memory use (bytes): %ld\n\n",
               m->vertices.maxitems * m->vertices.itembytes + m->triangles.maxitems * m->triangles.itembytes +
                   m->subsegs.maxitems * m->subsegs.itembytes + m->viri.maxitems * m->viri.itembytes +
                   m->badsubsegs.maxitems * m->badsubsegs.itembytes + m->badtriangles.maxitems * m->badtriangles.itembytes +
                   m->flipstackers.maxitems * m->flipstackers.itembytes + m->splaynodes.maxitems * m->splaynodes.itembytes);
